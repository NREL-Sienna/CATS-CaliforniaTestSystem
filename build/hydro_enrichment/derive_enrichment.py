...
.claude/plans/2026-08-17-cats-hydro-enrichment-design.md. Reads:
  ...
  - build/generator_types.jl   (current PM_TYPE_DICT, parsed not duplicated)
...
Run from anywhere: `python3 build/hydro_enrichment/derive_enrichment.py`
"""
import csv
import datetime
import re
...
GENERATOR_TYPES_JL = REPO_ROOT / "build" / "generator_types.jl"
...
def classify(fuel_type):
    """Mirrors the FuelType branching in build/build_CATS.jl's STEP 1 loop,
...
def write_report(**ctx):
    ...
    p()
    p(
        "Generated by `build/hydro_enrichment/derive_enrichment.py`. Regenerate "
        "after any change to `GIS/CATS_gens.csv`, `Archive/EIA_Generator_Y2019.csv`, "
        "or `build/generator_types.jl`; do not hand-edit."
    )
    p()
